...
class ChaoticNodes implements Runnable {
    ...
    ChaoticNodes(String threadname, int MaxFrameNumber, int applicationType) {
                ...
                if (NumberOfAttempts < 15) { //15 is the maximum number of attempts
                    try {
                        if (channel.isInUse()) {
                            ...
                            chosenStrategy++;
                            NetworkVariables networkVariables = new NetworkVariables(NumberOfAttempts, applicationType, chosenStrategy);
